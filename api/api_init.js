...
function initApp() {
	...
	//initdb().then(function(db) {
		...
		router.post('/guid', (req, res) => {
			...
			if (req.body.guid && req.body.guid.length > 0) {
				initdb().then(function(db) {
					db.collection('questions').findOne({guid:{$eq:req.body.guid}}).then(function(question) {
		...
		router.post('/mcguid', (req, res) => {
			//console.log(['mcguid',req.body.guid]);
			
			if (req.body.guid && req.body.guid.length > 0) {
				initdb().then(function(db) {
					db.collection('multiplechoicequestions').find({guid:{$eq:req.body.guid}}).toArray().then(function(questions) {
		...
		router.post('/importquestion', (req, res) => {
			...
			// min requirements
			if (req.user && req.body.question && req.body.question.length > 0 && req.body.importtype && req.body.importtype.length > 0) {
				initdb().then(function(db) {
					...
					function saveToReviewFeed(user,question) {
						...
						db.collection('seen').insertOne({user:ObjectId(user),question:ObjectId(question._id),timestamp:ts}).then(function(inserted) {
						   //console.log(['seen inserted']);
							...
							updateQuestionTallies(req.user._id,question._id).then(function() {
					...
					db.collection('questions').findOne({$and:[
						...
						if (question) {
							...
							db.collection('questions').updateOne({_id:question._id},{$set:Object.assign(question,req.body)}).then(function() {
								saveToReviewFeed(req.user._id,question);
							});
						} else {
		...
		router.post('/blocktopic', (req, res) => {
			if (req.user  && req.body.topic && req.body.topic.length > 0) {
				let criteria=[];
				if (req.user._id) {
					criteria.push({$or:[{access:{$eq:req.user._id}},{access:{$eq:'public'}}]})
				} else {
					criteria.push({access :{$eq:'public'}});
				}
				let topic = req.body.topic.trim(); //.toLowerCase(); 
					...
					db.collection('questions').find({$and:criteria}).sort({sort:1}).toArray(function(err, results) {
					  if (results) {
						  results.map(function(val,key) {
							  blockQuestion(req.user._id,val._id,topic);
						  });
		...
		router.post('/unblocktopic', (req, res) => {
			if (req.user && req.body.topic && req.body.topic.length > 0) {
				let criteria=[];
				criteria.push({user :{$eq:ObjectId(req.user._id)}});
				criteria.push({topic :{$eq:req.body.topic}});
		...
		//router.get('/recenttopics', (req, res) => {
			//if (req.user._id && req.user._id.length > 0) {
				//let collatedTopics={};
				////
			   //// $or:[{'successTally':{$lt : 7}},{'successTally':{$exists : false}}]
				//initdb().then(function(db) {
					...
					//db.collection('userquestionprogress').aggregate([
						//{ $match: {
								//$and:[{'user': {$eq:ObjectId(req.user._id)}}]
					   //}},
						//{ $group: {'_id': "$topic",
							//'questions': { $sum: 1 },
							//'topic': { $last: "$topic" },
					...
					//]).toArray().then(function(final) {
					   ////  console.log(['RECENT TOPICS',final]);
						...
						////console.log(['UPDATE USER TOTLAL' ,req.user._id,totalQuestions]);
						////db.collection('users').updateOne({_id:{$eq:ObjectId(req.user._id)}},{$set:{questions:totalQuestions}}).then(function() {
						  //////  console.log('UPDATED');
						////});
					   //// //console.log(['topics',topics]);
						...
						//db.collection('questions').aggregate([
							...
							//{ $group: {'_id': "$quiz",
								...
								//'topic': { '$last': "$quiz" }
							//}}
						//]).toArray().then(function(questionFinal) {
	...
	////                        questionFinal.values().map(function(val) {
		...
		router.post('/recenttopics', async (req, res) => {
			//console.log(['aarec topics' ,req.body, req.user]);
			if (req.user) {
				let collatedTopics={};
				//
			   // $or:[{'successTally':{$lt : 7}},{'successTally':{$exists : false}}]
				initdb().then(async function(db) {
					//console.log(['recemt tofpfics found user ',req.body.user])
					if (req.user) { 
						//  collate all user progress by topic
						var aggCursor = db.collection('userquestionprogress').aggregate([
							{ $match: {
									$and:[{'user': ObjectId(req.user._id)}]
						   }},
							{ $group: {'_id': "$topic",
								'questions': { $sum: 1 },
								'topic': { $last: "$topic" },
						...
						let topics = [];
						let totalQuestions = 0;
						//console.log('userquestionprogress agg cursor',aggCursor)
						await aggCursor.forEach(function(val,key) {
							//console.log('userquestionprogress agg it',val,key)
							// require topic, skip archived (success > 0.7)
							if (val.successRate<0.8 && val.topic && String(val.topic).length > 0) {
								collatedTopics[val.topic]={_id:val.topic,topic:val.topic,questions:val.questions,successRate:val.successRate,blocks:val.blocks}
								//console.log(['push TOPICS',val.topic]);
						...
						//console.log(['recemt topics found tp[ocs ',topics])
						//// UPDATE USERS WITH TALLY
						//console.log(['UPDATE USER TOTLAL' ,topics,totalQuestions]);
						//db.collection('users').updateOne({_id:{$eq:ObjectId(req.user._id)}},{$set:{questions:totalQuestions}}).then(function() {
						  ////  console.log('UPDATED');
						//});
					   // //console.log(['topics',topics]);
						...
						// lookup all questions in those topics and set total(questions) for each for each collatedTopic
						var aggCursor = db.collection('questions').aggregate([
							{ $match: criteria
							},
							{ $group: {'_id': "$quiz",
								'questions': { $sum: 1 },
						...
						//console.log(['agg ' ,collatedTopics]);
						// filter fully blocked topics
						Object.keys(collatedTopics).map(function(key) {
							let val = collatedTopics[key];
							if (val.blocks < val.total) {
		...
		router.post('/archivedtopics', async (req, res) => {
			if (req.user ) {
				let collatedTopics={};
				...
				initdb().then(async function(db) {
				   ...
				   // //console.log(['topics',topics]);
					...
					var aggCursor2 = db.collection('questions').aggregate([
						{ $match: criteria
						},
						{ $group: {'_id': "$quiz",
							'questions': { $sum: 1 },
					...
					await aggCursor2.forEach(function(val) {
		...
		router.post('/blockedtopics', async (req, res) => {
			...
			if (req.user) {
				...
				initdb().then(async function(db) {
					var aggCursor = db.collection('userquestionprogress').aggregate([
						{ $match: {
								$and:[{'user': {$eq:ObjectId(req.user._id)}},{'block':{$eq : 1}}]
					   }},
						{ $group: {'_id': "$topic",
							'questions': { $sum: 1 },
							'topic': { $last: "$topic" },
		...
		router.post('/usersuccessprogress', (req, res) => {
			if (req.user ) {
				initdb().then(function(db) {
					db.collection('userquestionprogress').aggregate([
						{ $match: {$and:[{'user': {$eq:ObjectId(req.user._id)}} , {block:{ $not: { $gt: 0 } }}]}},
		...
		router.post('/topiccollections', (req, res) => {
		...
		router.post('/progress', (req, res) => {
			if (req.user) {
				db.collection('progress').findOne({user:req.user._id}).then(function(progress) {
					res.send(progress);
				}).catch(function(e) {
					//console.log(e);
					res.send({});
				...
				if (req.body.topic && req.body.topic.length > 0) {
					db.collection('userTopics').findOne({publishedTopic:req.body.topic}).then(function(userTopic) {
						if (userTopic && userTopic.topicpassword  && userTopic.topicpassword.length > 0) {
							if (req.user) {
								db.collection('users').findOne({_id:ObjectId(req.user._id)}).then(function(user) {
				...
				if (req.user) {
					...
					db.collection('users').find({_id:ObjectId(user)}).toArray().then(function(users) {
						if (users.length > 0) {
							...
							let orParts = [
								{access:{$eq:ObjectId(req.user._id)}},
								{access :{$eq:'public'}},
							];
							if (allowedTopicCriteria.length > 0) {
		...
		router.post('/review', (req, res) => {
			...
			//console.log('RRREV', req.user, req.local);
			...
			initdb().then(function(db) {
				...
				let limit=5;
				 ...
				 orderMeBy[orderBy] = 1;          
				 let criteria=[];
				 if (req.body.band && req.body.band.length > 0) {
					 //console.log('FILTER BY BAND '+req.body.band );
					 if (parseInt(req.body.band,10) > 0) {
						 criteria.push({successTally:{$eq:parseInt(req.body.band,10)}});
					 } else {
				 ...
				 //if ((req.body.topic && req.body.topic.length > 0) || (req.body.band && req.body.band.length > 0)) {
				...
				// criteria.push({block:{ $not: { $gt: 0 }}});
				 //console.log("UUUU",req.body.user);
				 if (req.user) {
					 criteria.push({user:ObjectId(req.user._id)});
					//console.log("CCC", JSON.stringify({$and:criteria}));
								  ...
								  // console.log('MISSING QUESTIONS');
								   for (let qid in completeCheck) {
									   if (completeCheck[qid] !== null) {
										   ...
										   db.collection("userquestionprogress").deleteOne({$and:[{question:{$eq:ObjectId(qid)}},{user:{$eq:ObjectId(req.user._id)}}]}).then(function(res) {console.log(res.result)});
									   }
								   }
							   }
		...
		router.post('/questions', (req, res) => {
			...
			let limit = 100;
			let skip = 0;
			if (req.body.limit && parseInt(req.body.limit,10) != NaN) {
			...
			initdb().then(function(db) {
				...
				function theRest() {
					//console.log(['THEREST',req.body.search,JSON.stringify(criteria)]);
				   // //console.log(['questions request',req.body.search,req.body.technique]);
					if (req.body.search && req.body.search.trim().length > 0) {
						// SEARCH BY technique and text query
						criteria.push({$text: {$search: req.body.search.trim()}});
						//criteria.push({question: {$regex: req.body.search.trim().toLowerCase()}});
						if (req.body.technique && req.body.technique.trim().length > 0) {
							criteria.push({'mnemonic_technique': {$eq:req.body.technique.trim()}});
					  ...
					  // console.log(criteria);
						...
						// SEARCH BY technique
						if (req.body.technique && req.body.technique.length > 0) {
							criteria.push({'mnemonic_technique': {$eq:req.body.technique}});
							db.collection('questions').find({$and:criteria}).limit(limit).skip(skip).project({score: {$meta: "textScore"}}).sort({score:{$meta:"textScore"}}).toArray(function(err, results) {
							  res.send({'questions':results});
							})
						// SEARCH BY topic
						} else if (req.body.missingMnemonicsOnly > 0) {
							//console.log(['search by missing mnem '+ req.body.topic]);
							...
							//})
						} else  if (req.body.topic && req.body.topic.length > 0) {
							////console.log(['topic search',req.body.topic,{'quiz': {$eq:req.body.topic}}]);
							let topic = req.body.topic.trim(); //.toLowerCase(); 
							criteria.push({'quiz': {$eq:topic}});
							//console.log(['topic search C    ',JSON.stringify(criteria)]);
							//if (req.body.missingMnemonicsOnly > 0) {
						...
						// SEARCH BY tag
						} else if (req.body.tag && req.body.tag.length > 0) {
							if (req.body.tag) { 
						...
						} else if (req.body.question && req.body.question.length > 0) {
		...
		router.post('/like', (req, res) => {
		   // //console.log(['like']);
			if (req.user && req.body.question && String(req.body.question).length > 0  && req.body.mnemonic && String(req.body.mnemonic).length > 0 ) {
			  //  //console.log(['ok']);
				let user = req.user._id
				let question = req.body.question;
				initdb().then(function(db) {
			...
			initdb().then(function(db) {
				db.collection('userquestionprogress').findOne({$and:[{'user': {$eq:ObjectId(user)}},{question:{$eq:ObjectId(question)}} ]}).then(function(progress) {
					if (progress) {
						...
						db.collection('userquestionprogress').updateOne({_id:progress._id},{$set:progress}).then(function() {
						  ...
						  progress.block = 1; //new Date().getTime();
						  progress.topic = topic;
						  db.collection('userquestionprogress').insertOne(progress).then(function() {
		...
		router.post('/block', (req, res) => {
			////console.log(['block']);
			if (req.user && req.body.question && String(req.body.question).length > 0 ) {
					...
					//	console.log(['update stats have user',user]);
						...
						if (user) {
							...
							//	console.log('UPDATING USER STATS '+(uniqueQuestionResults ? uniqueQuestionResults.length: 0));
								...
								db.collection('users').replaceOne({_id: ObjectId(userId)},user, { upsert: true }).then(function(res) {
		...
		router.post('/sendallquestionsforreview', (req, res) => {
			//console.log('sendallquestionsforreview',req.body,req.body.questions.length,req.body.questions)
			if (req.user && req.body.questions && req.body.questions.length > 0 ) {
			    //console.log(['sendallquestionsforreview']);
				let user = req.user._id
		...
		router.post('/markallreviewed', (req, res) => {
			if (req.user && req.user._id && req.user._id.length > 0 && req.body.questions && req.body.questions.length > 0 ) {
				//console.log(['sendallquestionsforreview',req.user._id,JSON.stringify(req.body.questions)]);
				let user = req.user._id
				let questions = req.body.questions;
				let ts = new Date().getTime();
				let seenRecords=[];
		...
		router.post('/success', (req, res) => {
			...
			if (req.user && req.body.question) {
				let user = req.user._id
				let question = req.body.question;
			    //console.log(['success',user,question]);
				  ...
				  ////      //console.log(['success update']);
					...
					//} else {
					  //  //console.log(['success insert']);
						let ts = new Date().getTime();
						initdb().then(function(db) {
							db.collection('successes').insertOne({user:ObjectId(user),question:ObjectId(question),timestamp:ts}).then(function(inserted) {
		...
		router.post('/tags', (req, res) => {
			...
			//if (req.body.title && req.body.title.length > 0) {
				//criteria[]
			//}
			let search=req.body.title.trim();   
			let sort= {value:1};
		...
		//router.get('/leaderboard', (req, res) => {
			////console.log(['LEADERBOARD',req.body.type]);
			//let sort={streak: -1,questions:-1, recall:-1};
			//if (req.body.type==="streak") {
				//sort = {streak: -1}
			//} else if (req.body.type==="questions") {
				//sort = {questions: -1}
			//} else if (req.body.type=="recall") {
				//sort = {recall:-1}
			//}
			//initdb().then(function(db) {
