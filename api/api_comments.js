...
function initRoutes(router,initdb) {
	...
	router.post('/reportproblem', (req, res) => {
		...
		if (req.body.question) {
			...
			let  link = 'https://mnemolibrary.com/discover/topic/'+question.quiz+'/'+question._id;
		...
		utils.sendMail(process.env.fromEmail,process.env.fromEmail,"Problem Content Report from Mnemo's Library ",content);
			...
			if (req.body.isReply && req.body.userEmail  && req.body.userEmail.length > 0) {
				if (req.body.userEmailPreferences && req.body.userEmailPreferences === "none") {
					...
					if (req.body.replies && req.body.replies.length > 0) {
						...
						let  link = process.env.websiteUrl + '/discover/topic/'+req.body.questionTopic+'/'+req.body.question;
						let  linkProfile = process.env.websiteUrl + '/profile';
