...
export default class SettingsPage extends Component {
	...
	//	else this.props.openLoginWindow()
	}   

    componentDidUpdate(props,state) {
		//console.log(['SETTINGSUPDATE',props.token,this.props.token])
		if (props.user !== this.props.user || (props.token !== this.props.token)) {
			if (this.props.user) this.setState({user:this.props.user})
			else this.props.openLoginWindow('settings')
		}
	}   
				...
				<div className="col-12">
				 <form method="POST" onSubmit={this.saveUser} className="form-group" autoComplete="false" >
					<div className='col-12 warning-message'>{this.state.warning_message}</div>
					  ...
					  <a id="edit"></a>
						<label htmlFor="username" className='row'>Email &nbsp;&nbsp;&nbsp;<b> {this.state.user.username}</b></label>
