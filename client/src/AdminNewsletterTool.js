...
export default class AdminNewsletterTool extends Component {
    ...
    constructor(props) {
        ...
        initialContent += '<br/><br/><a href="https://mnemolibrary.com/review">Revise questions from your review list</a>'
        initialContent += '<br/><a href="https://mnemolibrary.com/multiplechoicetopics">Try a multiple choice quiz</a>'
         initialContent += '<br/><br/>If you don\'t want to receive these emails.  <a href="https://mnemolibrary.com/settings">Update your settings</a>'
	...
	sendTestNewsletter() {
		let that = this;
		this.startWaiting();
		that.props.fetch('/api/sendtestnewsletter', {
		  method: 'POST',
		  headers: {
	...
	reallySendNewsletter(isTest,content) {
		let that = this;
		this.startWaiting()
		that.props.fetch('/api/publishnewsletter', {
		  method: 'POST',
		  headers: {
			'Content-Type': 'application/json'
		  },
	...
	newNewsletter() {
		...
		that.props.fetch("/api/savenewsletter",{method: 'POST',headers: {'Content-Type': 'application/json'},body: JSON.stringify({content:that.initialContent})
         }).then(function(response) {
			return response.json()
		}).then(function(json) {
		//	console.log(['LOADED SUBS',json])
