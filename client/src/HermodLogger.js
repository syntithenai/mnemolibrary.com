...
export default class HermodLogger  extends HermodMqttServer {
    ...
    constructor(props) {
        ...
        this.state={mqttServer:'ws://localhost:9001',sites:{},messages:[],session:{},audioListening:{},hotwordListening:{},showLogMessages:{},sessionStatus:{},sessionStatusText:{}};
        this.audioBuffers={};
        this.lastSessionId={};
		...
		if (topic && topic.length && typeof callback === "function") {
			...
			if (this.subscriptions.hasOwnProperty(topic)) {
				...
				if (!this.props.subscription || this.props.subscription.length === 0) {
					 console.log('DO SUB really'+topic)
    ...
    onMessageArrived(topic,message) {
		...
		if (parts.length > 1 && parts[0] === "hermod") {
			...
			session = that.getSession(siteId,payload ? payload.id : null);
