...
export default  class TopicPassword extends Component {
    ...
    continueToTopic(e) {
	 ...
	 saveUser() {
		...
		let newUser={_id:user._id,email:user.username,topicPasswords:user.topicPasswords}
...
console.log(['CHECKPW p',that.props.fetch]);
			
			that.props.fetch("/api/checktopicpassword", {
			   method: "POST",
			   headers: {
