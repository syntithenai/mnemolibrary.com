...
export default class FeedMuncher extends Component {
	...
	componentDidMount() {
		...
		that.props.fetch('/api/topics?title=%20').then(function(response) {
    ...
    setExpanded(key) {
		...
		if (this.state.items.length > key && this.state.items[key] && this.state.items[key].link) {
			...
			that.props.fetch('/api/pageproxy?url='+that.state.items[key].link.trim()).then(response => {
				return response.json()
    ...
    updateUrl(event,value) {
		...
		this.updateTimeout = setTimeout(function() {
			if (that.state.feedUrl) {
				...
				that.props.fetch('/api/feedproxy?url='+that.state.feedUrl.trim()).then(response => {
					return response.json()
				}).then(function(json) {
