...
export default class SiteMap extends Component {
    ...
    constructor(props) {
     ...
     dumpalexa() {
		this.hideTopButtons()
       // that.props.fetch('/api/dumpalexa', {});
    };
    ...
    clickImport() {
	     confirmAlert({
          ...
          buttons: [
            ...
            {
			  label: 'MC only',
              onClick: () => this.props.importMultipleChoice(2)
            },
            {
              label: 'Steve',
			...
			<div className="container" >
				{(that.props.isAdmin()) && <div className="row" style={blockStyle}>
				 ...
				 {this.props.isLoggedIn() && <div className="col"><Link style={buttonStyle} className='btn btn-info'  to="/profile"  >Profile</Link><Link style={buttonStyle} className='btn btn-info'  to="/settings"  >Settings</Link></div>}
				  ...
				  {this.props.isLoggedIn() &&  <div className="col"><Link style={buttonStyle} className='btn btn-success'  to="/create"  >Create</Link></div>}
				  {this.props.isLoggedIn() &&  <div className="col"><Link style={buttonStyle} className='btn btn-success'  to="/quickmemo"  >Quick Memo</Link></div>}
