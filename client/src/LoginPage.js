...
export default class LoginPage extends Component {
    ...
    componentDidMount() {
		//if (!this.props.user) this.props.openLoginWindow()
		//function getHashValue(key) {
			//var matches = location.hash.match(new RegExp(key+'=([^&]*)'));
			//return matches ? matches[1] : null;
		//}
		let url = this.props.loginUrl;
		let hash = this.props.loginCallback //getHashValue('state') 
		if (hash) {
			url = url + '&state=' + hash
		//	window.location = url
		}
		//console.log(['NBT',url,hash])
		window.location = url  //+ '&state=/settings';
	}
