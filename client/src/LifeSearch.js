...
export default class AtlasLivingAustraliaSearch extends Component {   
	...
	alaExpandData(questionKey) {
		...
		if (question && this.getScientificName(question).length > 0) {
            ...
            that.props.fetch('/api/feedproxy?url='+encodeURIComponent('https://practicalplants.org/w/api.php?format=xml&redirects=true&action=query&titles='+that.getScientificName(question))).then(function(response) {
				return response.json();
				...
				if (ppPart && ppPart.title  && ppPart.title.length > 0) {
					...
					that.props.fetch('/api/feedproxy?url=https://practicalplants.org/wiki/Special:ExportRDF/'+encodeURIComponent(ppPage)).then(function(response) {
						return response.json();
